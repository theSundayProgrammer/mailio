...
const std::string mime::CONTENT_ID_HEADER{"Content-ID"};
const std::string mime::ADDRESS_BEGIN_STR(1, ADDRESS_BEGIN_CHAR);
const std::string mime::ADDRESS_END_STR(1, ADDRESS_END_CHAR);
const std::string mime::MESSAGE_ID_REGEX = "([a-zA-Z0-9\\!#\\$%&'\\*\\+\\-\\./=\\?\\^\\_`\\{\\|\\}\\~]+)"
    "\\@([a-zA-Z0-9\\!#\\$%&'\\*\\+\\-\\./=\\?\\^\\_`\\{\\|\\}\\~]+)";
const std::string mime::MESSAGE_ID_REGEX_NS = "([a-zA-Z0-9\\!#\\$%&'\\*\\+\\-\\./=\\?\\^\\_`\\{\\|\\}\\~\\@\\\\ \\t<\\>]*)";
const std::string mime::content_type_t::ATTR_CHARSET{"charset"};
const std::string mime::content_type_t::ATTR_BOUNDARY{"boundary"};
...
mime::content_type_t::content_type_t(media_type_t media_type, const std::string& media_subtype, const std::string& charset)
{
...
mime::content_type_t::content_type_t(media_type_t media_type, const std::string& media_subtype, const attributes_t& attributes, const std::string& charset)
{
    media_type_ = media_type;
    media_subtype_ = to_lower_copy(media_subtype);
...
std::string mime::content_type_t::media_subtype() const
{
...
std::string mime::content_type_t::boundary() const
{
...
void mime::content_type_t::boundary(const std::string& bound)
{
...
void mime::content_type_t::add_attribute(std::string name, string_t value)
{
    attributes_[name] = value;
}


std::string mime::content_type_t::make_boundary() const
{
    std::string bound;
    static std::string::size_type BOUND_LEN = 10;
    bound.reserve(BOUND_LEN);
    std::random_device rng;
    std::uniform_int_distribution<> index_dist(0, codec::HEX_DIGITS.size() - 1);
    for (std::string::size_type i = 0; i < BOUND_LEN; i++)
...
const std::string mime::CONTENT_TYPE_HEADER{"Content-Type"};
const std::string mime::CONTENT_TRANSFER_ENCODING_HEADER{"Content-Transfer-Encoding"};
const std::string mime::CONTENT_TRANSFER_ENCODING_BASE64{"Base64"};
const std::string mime::CONTENT_TRANSFER_ENCODING_BIT7{"7bit"};
const std::string mime::CONTENT_TRANSFER_ENCODING_BIT8{"8bit"};
const std::string mime::CONTENT_TRANSFER_ENCODING_QUOTED_PRINTABLE{"Quoted-Printable"};
const std::string mime::CONTENT_TRANSFER_ENCODING_BINARY{"Binary"};
const std::string mime::CONTENT_DISPOSITION_HEADER{"Content-Disposition"};
const std::string mime::CONTENT_DISPOSITION_ATTACHMENT{"attachment"};
const std::string mime::CONTENT_DISPOSITION_INLINE{"inline"};
const std::string mime::NEW_LINE_INDENT{"  "};
const std::string mime::HEADER_SEPARATOR_STR{": "};
const std::string mime::NAME_VALUE_SEPARATOR_STR{"="};
const std::string mime::ATTRIBUTES_SEPARATOR_STR{"; "};
const std::string mime::ATTRIBUTE_CONTINUATION_INDICATOR{"*"};
const std::string mime::ATTRIBUTE_NAME{"name"};
const std::string mime::ATTRIBUTE_FILENAME{"filename"};
const std::string mime::BOUNDARY_DELIMITER(2, '-');
const std::string mime::QTEXT{"\t !#$%&'()*+,-.@/:;<=>?[]^_`{|}~"};
...
const std::string mime::CONTENT_ATTR_ALPHABET{"!#$%&'*+-.^_`|~"};
const std::string mime::CONTENT_HEADER_VALUE_ALPHABET{"!#$%&*+-./^_`|~"};
...
void mime::format(std::string& mime_str, bool dot_escape) const
{
    ...
    std::string content = format_content(dot_escape);
    mime_str += content;
...
{
    std::string ms = reinterpret_cast<const char*>(mime_str.c_str());
    format(ms, dot_escape);
}
#endif


void mime::parse(const std::string& mime_string, bool dot_escape)
{
    std::string::size_type line_begin = 0;
    std::string::size_type line_end = mime_string.find(codec::END_OF_LINE, line_begin);
    std::string line;
...
mime& mime::parse_by_line(const std::string& line, bool dot_escape)
{
    if (line.length() > std::string::size_type(line_policy_))
...
void mime::content_type(media_type_t media_type, const std::string& media_subtype, const std::string& charset)
{
...
void mime::content_type(media_type_t media_type, const std::string& media_subtype, const attributes_t& attributes,
...
void mime::content_id(std::string id)
{
...
void mime::content(const std::string& content_str)
{
...
{
    content_ = std::string(reinterpret_cast<const char*>(content_str.c_str()));
}
#endif


std::string mime::content() const
{
    return content_;
}
...
std::string mime::format_many_ids(const vector<std::string>& ids, const std::string& header_name) const
{
    std::string ids_str;
    ...
    std::string::size_type l1p = static_cast<std::string::size_type>(line_policy_) - header_name.length() - HEADER_SEPARATOR_STR.length();
    bit7 b7(l1p, static_cast<std::string::size_type>(line_policy_));
    vector<std::string> ids_enc = b7.encode(ids_str);
    std::string ids_folded = ids_enc.at(0) + codec::END_OF_LINE;
    ids_folded += fold_header_line(ids_enc);
    return ids_folded;
}


std::string mime::format_many_ids(const std::string& id, const std::string& header_name) const
{
    return format_many_ids(vector<std::string>{id}, header_name);
}


vector<std::string> mime::parse_many_ids(const std::string& ids) const
{
    if (!strict_mode_)
        return vector<std::string>{ids};

    vector<std::string> idv;
    auto start = ids.cbegin();
    auto end = ids.cend();
    match_flag_type flags = std::regex_constants::match_default | std::regex_constants::match_not_null;
    match_results<std::string::const_iterator> tokens;
    bool all_tokens_parsed = false;
    const std::string ANGLED_MESSAGE_ID_REGEX = codec::LESS_THAN_STR + MESSAGE_ID_REGEX + codec::GREATER_THAN_STR;
...
std::string mime::format_content(bool dot_escape) const
{
    ...
    switch (encoding_)
    {
        ...
        {
            base64 b(static_cast<std::string::size_type>(line_policy_), static_cast<std::string::size_type>(line_policy_));
        ...
        {
            quoted_printable qp(static_cast<std::string::size_type>(line_policy_), static_cast<std::string::size_type>(line_policy_));
        ...
        {
            bit8 b8(static_cast<std::string::size_type>(line_policy_), static_cast<std::string::size_type>(line_policy_));
        ...
        {
            bit7 b7(static_cast<std::string::size_type>(line_policy_), static_cast<std::string::size_type>(line_policy_));
        ...
        case content_transfer_encoding_t::BINARY:
        {
            ...
            binary b(static_cast<std::string::size_type>(line_policy_), static_cast<std::string::size_type>(line_policy_));
            b.strict_mode(strict_codec_mode_);
            content_lines = b.encode(content_);
            break;
        }
    ...
    for (const auto& s : content_lines)
        if (dot_escape && s[0] == codec::DOT_CHAR)
            content += std::string(1, codec::DOT_CHAR) + s + codec::END_OF_LINE;
        else
...
std::string mime::format_content_type() const
{
    std::string line;
    ...
    if (content_type_.media_type() != media_type_t::NONE)
    {
        ...
        {
            std::string attrs = split_attributes(ATTRIBUTE_NAME, name_);
...
std::string mime::format_transfer_encoding() const
{
    std::string line;
...
std::string mime::format_content_disposition() const
{
    std::string disp;
    ...
    std::string attrs = split_attributes(ATTRIBUTE_FILENAME, name_);
...
std::string mime::format_content_id() const
{
    return content_id_.empty() ? "" : CONTENT_ID_HEADER + HEADER_SEPARATOR_STR + format_many_ids(content_id_, CONTENT_ID_HEADER);
}


std::string mime::fold_header_line(const vector<std::string>& headers) const
{
    std::string hdr_str;
    if (headers.size() > 1)
...
void mime::parse_content()
{
    ...
    switch (encoding_)
    {
        case content_transfer_encoding_t::BASE_64:
        {
            base64 b64(static_cast<std::string::size_type>(line_policy_), static_cast<std::string::size_type>(line_policy_));
...
*/
void mime::parse_header_line(const std::string& header_line)
{
...
void mime::parse_header_name_value(const std::string& header_line, std::string& header_name, std::string& header_value) const
{
    std::string::size_type colon_pos = header_line.find(HEADER_SEPARATOR_CHAR);
...
void mime::parse_content_type(const std::string& content_type_hdr, media_type_t& media_type, std::string& media_subtype, attributes_t& attributes) const
{
    std::string header_value;
    parse_header_value_attributes(content_type_hdr, header_value, attributes);
    std::string media_type_s;
...
void mime::parse_content_transfer_encoding(const std::string& transfer_encoding_hdr, content_transfer_encoding_t& encoding, attributes_t& attributes) const
{
    std::string header_value;
...
void mime::parse_content_disposition(const std::string& content_disp_hdr, content_disposition_t& disposition, attributes_t& attributes) const
{
    std::string header_value;
...
*/
void mime::parse_header_value_attributes(const std::string& header, std::string& header_value, attributes_t& attributes) const
{
        ...
        switch (state)
        {
            case state_t::BEGIN:
                ...
                else if (isalpha(*ch) || isdigit(*ch))
                {
                    ...
                    throw mime_error("Parsing header value error.", "Syntax error at character `" + std::string(1, *ch) + "`, at position " + to_string(char_pos)
                        + ".\nHeader is `" + header + "`.");
                break;

            case state_t::VALUE:
                if (isalpha(*ch) || isdigit(*ch) || CONTENT_HEADER_VALUE_ALPHABET.find(*ch) != std::string::npos)
                    header_value += *ch;
            ...
            case state_t::ATTR_BEGIN:
                ...
                else if (isalpha(*ch) || isdigit(*ch) || CONTENT_ATTR_ALPHABET.find(*ch) != std::string::npos)
                {
                    ...
                    throw mime_error("Parsing attribute name error.", "Syntax error at character `" + std::string(1, *ch) + "`, at position " + to_string(char_pos)
                        + ".\nHeader is `" + header + "`.");
                break;
            ...
            case state_t::ATTR_SEP:
                ...
                else if (isalpha(*ch) || isdigit(*ch) || CONTENT_ATTR_ALPHABET.find(*ch) != std::string::npos)
                {
                    ...
                    throw mime_error("Parsing attribute value error.", "Syntax error at character `" + std::string(1, *ch) + "`, at position " +
                        to_string(char_pos) + ".\nHeader is `" + header + "`.");
                break;
            ...
            case state_t::QATTR_VALUE_BEGIN:
                if (isalpha(*ch) || isdigit(*ch) || QTEXT.find(*ch) != std::string::npos)
                    attribute_value += *ch;
            ...
            case state_t::ATTR_VALUE_BEGIN:
                ...
                else if (*ch == ATTRIBUTES_SEPARATOR_CHAR)
                {
                    ...
                    throw mime_error("Parsing attribute value error.", "Syntax error at character `" + std::string(1, *ch) + "` at position " + to_string(char_pos)
                        + ".\nHeader is `" + header + "`.");
                if (ch == header.end() - 1)
                {
                    attributes[attribute_name] = attribute_value;
...
std::string mime::split_attributes(const std::string& attr_name, const string_t& attr_value) const
{
    ...
    const std::string::size_type CHARSET_LANG_EQ_SC = 9;
    ...
    const std::string::size_type PART_MARK = 2;
    const std::string::size_type line1_policy = static_cast<std::string::size_type>(line_policy_) - (attr_name.length() + PART_MARK) - CHARSET_LANG_EQ_SC -
        ATTRIBUTES_SEPARATOR_STR.length();
    // Deduce two tab characters for the folding and two for double quotes.
    const std::string::size_type FOLD_QUOTES = 4;
    const std::string::size_type line_policy = static_cast<std::string::size_type>(line_policy_) - (attr_name.length() + PART_MARK) - FOLD_QUOTES -
        ATTRIBUTES_SEPARATOR_STR.length();
    vector<std::string> attr_parts;
    ...
    {
        std::string attr_str = NEW_LINE_INDENT + attr_name;
    ...
    const vector<std::string>::size_type MAX_PARTS = 10;
    vector<std::string>::size_type total_parts = std::min(MAX_PARTS, attr_parts.size());
    vector<std::string>::size_type part_no = 0;
    std::string attrs;
...
string_t mime::decode_value_attribute(const std::string& attr_value) const
{
    if (attr_value.empty())
        return string_t();
    ...
    if (charset_pos != std::string::npos)
    {
        ...
        percent pct(static_cast<std::string::size_type>(line_policy_), static_cast<std::string::size_type>(line_policy_));
...
std::string mime::mime_type_as_str(media_type_t media_type_val) const
{
    std::string mime_str;
...
mime::media_type_t mime::mime_type_as_enum(const std::string& media_type_val) const
{
