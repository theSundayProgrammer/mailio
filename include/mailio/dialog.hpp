...
public:
    ...
    @throw *             `connect_async()`.
    **/
    dialog(asio::io_context& ios, const std::string& hostname, unsigned port, std::chrono::milliseconds timeout);
...
protected:
    ...
    **/
    void timeout_handler(const std::error_code& error);
    ...
    **/
    std::shared_ptr<asio::streambuf> strmbuf_;
...
public:
    ...
    **/
    dialog_ssl(asio::io_context& ios, const std::string& hostname, unsigned port, std::chrono::milliseconds timeout, const ssl_options_t& options);
